...
public class TicTacToe {
    ...
    private static boolean isCellValid(int x, int y) {
        return (x >= 0 && y >= 0 && x < SIZE && y < SIZE && map[x][y] == DOT_U);
    }
    ...
    private static int isGameOver(char c) {
        /*
         * ничья -> 0
         * игрок с символом c победил -> 1
         * игра продолжается -> 2
         */
        //проверяем все линии
        ...
        for (int i = 0; i < SIZE; i++) {
            if (map[i][i] == c) d1++;
            if (map[SIZE - i - 1][i] == c) d2++;

            int h = 0, v = 0;  // SIZE раз посчитаем кол-во X/O на горизонтальных и вертикальных линиях
            for (int j = 0; j < SIZE; j++) {
                if (map[i][j] == c) h++;
                if (map[j][i] == c) v++;
            }
        ...
        //проверяем нет ли свободных полей
        for (int i = 1; i < SIZE; i++)
            for (int j = 0; j < SIZE; j++)
                if (map[i][j] == DOT_U) return 2;
        return 0;
    }
